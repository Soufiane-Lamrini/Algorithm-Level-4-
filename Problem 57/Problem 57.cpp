...
enum enClassementDate {
    Avant = -1, Egale = 0, Apres = 1
};
enClassementDate ComparerDate(sDate Date1, sDate Date2)
{
    if (Date1AvantDate2(Date1, Date2))

        return enClassementDate::Avant;
    if (Date1EgalDate2(Date1, Date2))

        return enClassementDate::Egale;
    
    return enClassementDate::Apres;
}
int main()
{
    ...
    cout << "resultats de comparaison " << ComparerDate(Date1, Date2) << endl;
