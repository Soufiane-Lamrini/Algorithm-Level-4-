...
enum enDateCompare { Avant = -1, Equal = 0, Apres = 1 };
enDateCompare ComparaisonDates(sDate Date1, sDate Date2)
{
    if (Date1AvantDate2(Date1, Date2))
        return enDateCompare::Avant;
    if (Date1EgalDate2(Date1, Date2))
        return enDateCompare::Equal;

    return enDateCompare::Apres;
}

bool EstSeIncluant(sPeriod p1, sDate Date)
{
    return !(ComparaisonDates(p1.DateDebut, Date) == enDateCompare::Apres || ComparaisonDates(p1.DateFin, Date) == enDateCompare::Avant);
...
int main()
{
    ...
    if (EstSeIncluant(Period1, Date))
        cout << "La periode est incluent la date." << endl;
    else
        cout << "La date n'appartient pas a la periode." << endl;
    system("pause>0");
    return 0;
}
